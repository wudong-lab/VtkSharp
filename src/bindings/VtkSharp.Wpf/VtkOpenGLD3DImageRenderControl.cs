...
public sealed class VtkOpenGLD3DImageRenderControl : FrameworkElement, IDisposable
{
    ...
    public void Render()
    {
        ...
        try
        {
            ...
            VtkWpfOpenGLD3DImageRenderBridge_SetSize(this._bridge, pixelSize.Width, pixelSize.Height);
            this._pixelSize = pixelSize;

            var backBuffer = VtkWpfOpenGLD3DImageRenderBridge_GetBackBuffer(this._bridge);
            ...
            VtkWpfOpenGLD3DImageRenderBridge_Render(this._bridge);
    ...
    private void InitializeBridge()
    {
        ...
        this._bridge = VtkWpfOpenGLD3DImageRenderBridge_New();
        if (this._bridge == IntPtr.Zero)
        {
            var nativeError = Marshal.PtrToStringAnsi(VtkWpfOpenGLD3DImageRenderBridge_GetLastError());
        ...
        this.RenderWindow = vtkRenderWindow.WeakReference(VtkWpfOpenGLD3DImageRenderBridge_GetRenderWindow(this._bridge));
        this.Renderer = vtkRenderer.WeakReference(VtkWpfOpenGLD3DImageRenderBridge_GetRenderer(this._bridge));
    ...
    private void DisposeBridge()
    {
        if (this._bridge != IntPtr.Zero)
        {
            ...
            VtkWpfOpenGLD3DImageRenderBridge_Delete(this._bridge);
    ...
    private static extern nint VtkWpfOpenGLD3DImageRenderBridge_New();

    [DllImport(NativeLibraryName)]
    private static extern void VtkWpfOpenGLD3DImageRenderBridge_Delete(nint bridge);

    [DllImport(NativeLibraryName)]
    private static extern nint VtkWpfOpenGLD3DImageRenderBridge_GetRenderWindow(nint bridge);

    [DllImport(NativeLibraryName)]
    private static extern nint VtkWpfOpenGLD3DImageRenderBridge_GetRenderer(nint bridge);

    [DllImport(NativeLibraryName)]
    private static extern void VtkWpfOpenGLD3DImageRenderBridge_SetSize(nint bridge, int width, int height);

    [DllImport(NativeLibraryName)]
    private static extern void VtkWpfOpenGLD3DImageRenderBridge_Render(nint bridge);

    [DllImport(NativeLibraryName)]
    private static extern nint VtkWpfOpenGLD3DImageRenderBridge_GetBackBuffer(nint bridge);

    [DllImport(NativeLibraryName)]
    private static extern nint VtkWpfOpenGLD3DImageRenderBridge_GetLastError();
