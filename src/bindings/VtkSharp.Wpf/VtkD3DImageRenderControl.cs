...
/// <summary>
/// Diagnostic fallback that renders VTK offscreen and uploads frames into a D3DImage back buffer.
/// </summary>
public sealed class VtkD3DImageRenderControl : FrameworkElement, IDisposable
{
    ...
    public VtkD3DImageRenderControl()
    {
        this.Focusable = true;
    ...
    public void Render()
    {
        ...
        try
        {
            ...
            VtkWpfD3DImageRenderBridge_SetSize(this._bridge, pixelSize.Width, pixelSize.Height);
            this._pixelSize = pixelSize;

            var backBuffer = VtkWpfD3DImageRenderBridge_GetBackBuffer(this._bridge);
            ...
            VtkWpfD3DImageRenderBridge_Render(this._bridge);
    ...
    private void InitializeBridge()
    {
        ...
        this._bridge = VtkWpfD3DImageRenderBridge_New();
        if (this._bridge == IntPtr.Zero) throw new InvalidOperationException("Failed to create VTK D3DImage render bridge.");

        this.RenderWindow = vtkRenderWindow.WeakReference(VtkWpfD3DImageRenderBridge_GetRenderWindow(this._bridge));
        this.Renderer = vtkRenderer.WeakReference(VtkWpfD3DImageRenderBridge_GetRenderer(this._bridge));
    ...
    private void DisposeBridge()
    {
        if (this._bridge != IntPtr.Zero)
        {
            ...
            VtkWpfD3DImageRenderBridge_Delete(this._bridge);
    ...
    private static extern nint VtkWpfD3DImageRenderBridge_New();

    [DllImport(NativeLibraryName)]
    private static extern void VtkWpfD3DImageRenderBridge_Delete(nint bridge);

    [DllImport(NativeLibraryName)]
    private static extern nint VtkWpfD3DImageRenderBridge_GetRenderWindow(nint bridge);

    [DllImport(NativeLibraryName)]
    private static extern nint VtkWpfD3DImageRenderBridge_GetRenderer(nint bridge);

    [DllImport(NativeLibraryName)]
    private static extern void VtkWpfD3DImageRenderBridge_SetSize(nint bridge, int width, int height);

    [DllImport(NativeLibraryName)]
    private static extern void VtkWpfD3DImageRenderBridge_Render(nint bridge);

    [DllImport(NativeLibraryName)]
    private static extern nint VtkWpfD3DImageRenderBridge_GetBackBuffer(nint bridge);
