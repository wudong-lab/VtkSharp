...
```text
1. parser / header scanner / hierarchy reader 发现 API
2. rule engine 对确定规则的 API 自动生成 generated manifest
3. AI 对不确定 API 给出候选 manifest、证据和风险说明
4. 生成 decisions / diagnostics 文件
5. 人工选择 accepted / rejected / needs-review
6. accepted 决策进入 overrides 或 decisions
7. resolve-manifest 合并生成 resolved manifest
8. generate 根据 resolved manifest 生成代码
```
