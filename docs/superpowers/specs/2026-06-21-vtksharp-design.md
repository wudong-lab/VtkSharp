...
本文是 VtkSharp 项目的总体技术基线，记录项目定位、C ABI 路线、MVP 范围、生命周期、测试策略等全局设计。

`2026-06-23-vtksharp-generator-design.md` 是源代码生成器专项设计，记录白名单格式、CLI 架构、类型映射、TFM 条件互操作、CMake 片段和示例驱动补全流程。

后续实现源代码生成器时，以 `2026-06-23-vtksharp-generator-design.md` 为准；如果本文中生成器、manifest 或 API 发现流程相关内容与该专项设计存在冲突，以专项设计为准。
