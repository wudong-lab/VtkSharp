param(
    ...
    [string]$WebColorUrl = "https://www.w3schools.com/tags/ref_colornames.asp",
...
$atomicWords = @(
    ...
    "golden", "rod", "gold", "gray", "grey", "pink", "tan", "teal", "aqua", "navy",
...
function Read-WebColorRows {
    param([string]$Html)

    $pattern = '<span class="colornamespan"><a[^>]*>(.*?)</a></span>\s*<br>\s*<div><span class="colorhexspan"><a[^>]*>#([0-9A-Fa-f]{6})</a></span>'
    $matches = [regex]::Matches($Html, $pattern, [Text.RegularExpressions.RegexOptions]::Singleline)

    foreach ($match in $matches) {
        $name = [Net.WebUtility]::HtmlDecode($match.Groups[1].Value.Trim())
        if ($name.EndsWith("Grey", [StringComparison]::Ordinal)) {
            continue
        }
...
$vtkResponse = Invoke-WebRequest -Uri $Url -UseBasicParsing
$html = $vtkResponse.Content
$webResponse = Invoke-WebRequest -Uri $WebColorUrl -UseBasicParsing
$webHtml = $webResponse.Content

$webRows = @(Read-WebColorRows $webHtml)
...
$lines.Add("")
$lines.Add("/// <summary>")
$lines.Add("/// VTK颜色值，对应<c>vtkColor3d</c>（RGB，分量范围 0.0–1.0）。")
$lines.Add("/// <para>")
$lines.Add("/// 静态预设值来源于 W3Schools HTML color names")
$lines.Add("/// 以及与 Web 颜色无同值别名冲突的 VTK color Names；Web Grey 拼写别名和 ParaView color Names 不包含在内。")
$lines.Add("/// </para>")
$lines.Add("/// </summary>")
...
$lines.Add("")
$lines.Add("    /// <summary>")
$lines.Add("    /// 命名颜色常量；Web 颜色字段名保留 W3Schools 页面中的大小写（如 <see cref=`"FireBrick`"/>）。")
$lines.Add("    /// </summary>")
