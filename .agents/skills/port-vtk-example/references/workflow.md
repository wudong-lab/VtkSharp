...
先读终端摘要，不必加载完整成功报告。需要详细签名时用 `tools/read-binding-report.ps1 -Path <report.json> -Class <VTKClass>` 或 `-Status needs-metadata` 精确筛选。合并前仍以当前 `diff-whitelist --summary` 为准，它只省略已存在项，保留全部新增 API、依赖和冲突。
...
# 目标示例实现 ISmokeExample 时，上述命令加 -Example <Category/Name>
```

生成代码编译失败时修正 whitelist 元数据、类型映射或 generator，不手改生成文件。

统一验证脚本包含 `generate-bindings --check`，失败时先看摘要，再读对应阶段日志，不重复运行已经通过且输入未变的检查。运行目标示例，人工确认首帧、相机、交互与关闭释放；自动截图通过不代表这些项目全部通过。自动验收接入方式及报告语义见 `docs/workflow/verification.md`。

在示例目录维护 `porting-notes.md`：引用原始源码，记录翻译差异、排除项、验证报告路径及实际结果；新增 API 与依赖清单优先引用工具报告，不重新抄写。对本次人工确认的方向、长度和所有权，按 `docs/workflow/interop-evidence.md` 保存源码版本、文件哈希和决定，复用前核对适用条件。最后审查 git diff，确认没有无关扩张或生成代码手改。
