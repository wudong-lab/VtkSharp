...
[Example("WPF D3DImage CPU Fallback", "WPF",
    Description = "Diagnostic fallback that uploads offscreen VTK frames into a D3DImage surface.",
    SourceFiles = new[] { "ExtraExamples/WpfD3DImageViewport/WpfD3DImageViewport.cs" })]
internal sealed class WpfD3DImageViewport : IExample
{
    public void Run()
    {
        ...
        dispatcher.Invoke(() =>
        {
            var window = new WpfD3DImageViewportWindow
            {
    ...
    private sealed class WpfD3DImageViewportWindow : Window
    {
        private vtkConeSource? _cone;
        private vtkPolyDataMapper? _mapper;
        private vtkActor? _actor;

        public WpfD3DImageViewportWindow()
        {
            this.Title = "VtkSharp WPF D3DImage Viewport";
            this.Width = 1000;
            this.Height = 720;
            this.MinWidth = 480;
            this.MinHeight = 360;

            var viewport = new VtkD3DImageRenderControl();
            viewport.VtkInitialized += this.OnVtkInitialized;

            var overlayButton = new Button
            {
                Content = "D3DImage Overlay",
        ...
        private void OnVtkInitialized(object? sender, VtkRenderControlInitializedEventArgs e)
        {
            ...
            this._actor.GetProperty().SetColor(0.2, 0.85, 0.55);

            e.Renderer.SetBackground(0.08, 0.1, 0.13);
            e.Renderer.AddActor(this._actor);
            e.Renderer.ResetCamera();

            if (sender is VtkD3DImageRenderControl control)
            {
                control.Render();
            }
        }
